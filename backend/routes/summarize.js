...
// Route 1 : Summarize Text. POST '/summarize/summarize_text'. Login Required
// summarize_text() is DEPRECATED. This route is not supposed to be used anymore.
// router.post('/summarize_text', (req, res) => {
...
// Route 2 : Summarize PDF, 3 versions : gist/one page/abridged/progressive summary. POST '/summarize/summarize_pdf'. Login Required
const pdfExtract = new PDFExtract();
const upload = multer({ storage: multer.memoryStorage() });
router.post('/summarize_pdf', authMiddleware, upload.single('pdf'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        if (existingBook) {
            ...
            if (timeDifference < oneWeekInMs) {
                ...
                logger(`User ${userId} tried to summarize before 1 week cooldown`, LOG_TYPES.WARNING);
                return res.status(429).json({ 
                    error: `You can only summarize one book per week. Please wait ${daysRemaining} more day(s).`,
                    old_summary: existingBook.styled_summary
                });
            }
...
The story sets the stage for a narrative centered on the clash between the mundane and the magical, between the desire for normalcy and the inevitability of change. The Dursleys’ rigid adherence to their self-imposed standards of normality creates a fertile ground for conflict, as the forces they attempt to suppress inevitably break through, disrupting their carefully controlled world and exposing the fragility of their carefully constructed reality. The initial quiet and order of Privet Drive belies the extraordinary events that are about to unfold, signaling the imminent arrival of a force that will forever alter the Dursleys' lives.
                ...
                const book = await prisma.book.create({
                    data: {
                        userId,
                        summary,
                        styled_summary: html_summary
                    }
                });
                
                logger(`Book created for user ${userId}: ${book.id}`, LOG_TYPES.SUCCESS);
                res.send({ summary: html_summary, bookId: book.id });
...
// Route 3 : Get Audio Link for summary stored. GET '/summarize/audioLink'. Login Required
router.get("/audioLink", authMiddleware, async (req, res) => {
    ...
    res.on('close', () => {
        logger('client dropped connection!', LOG_TYPES.INFORMATION);
        res.end();
    });
    ...
    if(book){   
        ...
        timeDifference = now - bookCreatedAt;
    }
    
    if (!book) {
        logger(`User ${userId} has no book to convert to audio`, LOG_TYPES.WARNING);
        res.write("event: error_no_book\n");
        res.write(`data: {"error": "No book found. Please upload and summarize a book first."}\n`);
        res.write('\n\n');
        return; // Client is expected to close the connection in receiving the "error_no_book" event as this is SSE. So, don't do "return res.end();" here.
    }
    else if(!book.audio_link || timeDifference > oneWeekInMs){
        try{
            ...
            logger(`Speech Synthesis task started. TaskId: ${TaskId} \n`, LOG_TYPES.SUCCESS);
                    ...
                    setTimeout(async ()=>{
                        ...
                        try{
                            ...
                            if(aws_response.SynthesisTask.TaskStatus === "completed"){
                                // Update book with audio link
                                await prisma.book.update({
                                    where: { id: book.id },
                                    data: { audio_link: aws_response.SynthesisTask.OutputUri }
                                });
                                res.write("event: audio_synthesis_done\n")
                                res.write(`data: {"audio_url": "${aws_response.SynthesisTask.OutputUri}"}`)
                                res.write('\n\n')
                                logger(`Audio link saved for book ${book.id}`, LOG_TYPES.SUCCESS);
                                endPoll = true;
                            ...
                            logger(`Error retrieving task. TaskID might be incorrect: ${error}`, LOG_TYPES.ERROR);
            ...
            logger(`Error in starting audio synthesis task: ${error}`, LOG_TYPES.ERROR);
        ...
        res.write("event: audio_synthesis_done\n")
        res.write(`data: {"audio_url": "${book.audio_link}"}`)
        res.write('\n\n')
    }
});
